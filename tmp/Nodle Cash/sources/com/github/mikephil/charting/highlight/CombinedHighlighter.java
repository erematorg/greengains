...
public class CombinedHighlighter extends ChartHighlighter<CombinedDataProvider> implements IHighlighter {
    ...
    public List<Highlight> getHighlightsAtXValue(float f2, float f3, float f4) {
        ...
        for (int i3 = 0; i3 < allData.size(); i3++) {
            ChartData chartData = allData.get(i3);
            ...
            if (barHighlighter2 == null || !(chartData instanceof BarData)) {
                ...
                for (int i4 = 0; i4 < dataSetCount; i4++) {
                    ...
                    if (dataSetByIndex.isHighlightEnabled()) {
                        for (Highlight next : buildHighlights(dataSetByIndex, i4, f2, DataSet.Rounding.CLOSEST)) {
                            next.setDataIndex(i3);
                            this.mHighlightBuffer.add(next);
                        }
                    }
                }
            } else {
                Highlight highlight = barHighlighter2.getHighlight(f3, f4);
                if (highlight != null) {
