...
public class Registry {
    ...
    public static class NoModelLoaderAvailableException extends MissingComponentException {
        ...
        public <M> NoModelLoaderAvailableException(@NonNull M m3, @NonNull List<ModelLoader<M, ?>> list) {
    ...
    public static class NoResultEncoderAvailableException extends MissingComponentException {
        public NoResultEncoderAvailableException(@NonNull Class<?> cls) {
            super(b.l("Failed to find result encoder for resource class: ", cls, ", you may need to consider registering a new Encoder for the requested type or DiskCacheStrategy.DATA/DiskCacheStrategy.NONE if caching your transformed resource is unnecessary."));
        }
    }

    public static class NoSourceEncoderAvailableException extends MissingComponentException {
        public NoSourceEncoderAvailableException(@NonNull Class<?> cls) {
            super(b.k("Failed to find source encoder for data class: ", cls));
        }
    }
        ...
        if (list == null) {
            ...
            for (Class<?> resourceClasses : this.modelLoaderRegistry.getDataClasses(cls)) {
                for (Class next : this.decoderRegistry.getResourceClasses(resourceClasses, cls2)) {
                    if (!this.transcoderRegistry.getTranscodeClasses(next, cls3).isEmpty() && !arrayList2.contains(next)) {
                        arrayList2.add(next);
                    }
                }
            }
    ...
    @NonNull
    public <X> DataRewinder<X> getRewinder(@NonNull X x2) {
        return this.dataRewinderRegistry.build(x2);
    }

    @NonNull
    public <X> Encoder<X> getSourceEncoder(@NonNull X x2) throws NoSourceEncoderAvailableException {
        Encoder<X> encoder = this.encoderRegistry.getEncoder(x2.getClass());
        if (encoder != null) {
            return encoder;
        }
