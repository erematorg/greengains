...
public class SizeConfigStrategy implements LruPoolStrategy {
    ...
    public static class KeyPool extends BaseKeyPool<Key> {
        public Key get(int i3, Bitmap.Config config) {
            Key key = (Key) get();
            key.init(i3, config);
            return key;
        }
    ...
    private Key findBestKey(int i3, Bitmap.Config config) {
        Key key = this.keyPool.get(i3, config);
        Bitmap.Config[] inConfigs = getInConfigs(config);
        int length = inConfigs.length;
        int i4 = 0;
        while (i4 < length) {
            Bitmap.Config config2 = inConfigs[i4];
            Integer ceilingKey = getSizesForConfig(config2).ceilingKey(Integer.valueOf(i3));
    ...
    public static String getBitmapString(int i3, Bitmap.Config config) {
        return "[" + i3 + "](" + config + ")";
    }
    ...
    private static Bitmap.Config[] getInConfigs(Bitmap.Config config) {
        ...
        return i3 != 1 ? i3 != 2 ? i3 != 3 ? i3 != 4 ? new Bitmap.Config[]{config} : ALPHA_8_IN_CONFIGS : ARGB_4444_IN_CONFIGS : RGB_565_IN_CONFIGS : ARGB_8888_IN_CONFIGS;
    }
    ...
    public Bitmap get(int i3, int i4, Bitmap.Config config) {
        Key findBestKey = findBestKey(Util.getBitmapByteSize(i3, i4, config), config);
    ...
    public void put(Bitmap bitmap) {
        ...
        sizesForConfig.put(valueOf, Integer.valueOf(i3));
    }
    ...
    public String toString() {
        StringBuilder p2 = a.p("SizeConfigStrategy{groupedMap=");
        p2.append(this.groupedMap);
        p2.append(", sortedSizes=(");
        for (Map.Entry next : this.sortedSizes.entrySet()) {
            p2.append(next.getKey());
            p2.append(AbstractJsonLexerKt.BEGIN_LIST);
    ...
    public static final class Key implements Poolable {
        ...
        public void init(int i3, Bitmap.Config config2) {
            this.size = i3;
            this.config = config2;
        }
        ...
        @VisibleForTesting
        public Key(KeyPool keyPool, int i3, Bitmap.Config config2) {
            this(keyPool);
            init(i3, config2);
        }
    }
    ...
    public String logBitmap(int i3, int i4, Bitmap.Config config) {
        return getBitmapString(Util.getBitmapByteSize(i3, i4, config), config);
    }
}
