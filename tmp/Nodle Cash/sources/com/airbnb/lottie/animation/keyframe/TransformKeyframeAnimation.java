...
public class TransformKeyframeAnimation {
    ...
    public <T> boolean applyValueCallback(T t2, @Nullable LottieValueCallback<T> lottieValueCallback) {
        if (t2 == LottieProperty.TRANSFORM_ANCHOR_POINT) {
            ...
            if (t2 == LottieProperty.TRANSFORM_SCALE) {
                ...
                return true;
            } else if (t2 != LottieProperty.TRANSFORM_SKEW_ANGLE) {
                return false;
            } else {
    ...
    public Matrix getMatrix() {
        ...
        if (!this.autoOrient) {
            ...
            this.matrix.preRotate((float) Math.toDegrees(Math.atan2((double) (value5.y - f4), (double) (value5.x - f3))));
        }
        FloatKeyframeAnimation floatKeyframeAnimation = this.skew;
        if (floatKeyframeAnimation != null) {
            FloatKeyframeAnimation floatKeyframeAnimation2 = this.skewAngle;
        ...
        if (!(baseKeyframeAnimation4 == null || (value = baseKeyframeAnimation4.getValue()) == null)) {
            ...
            if (!(f6 == 0.0f && value.y == 0.0f)) {
                this.matrix.preTranslate(-f6, -value.y);
            }
        }
        return this.matrix;
    }
