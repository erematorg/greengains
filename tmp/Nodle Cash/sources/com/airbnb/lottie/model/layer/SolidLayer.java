...
public class SolidLayer extends BaseLayer {
    ...
    public <T> void addValueCallback(T t2, @Nullable LottieValueCallback<T> lottieValueCallback) {
        ...
        } else if (t2 != LottieProperty.COLOR) {
        } else {
            if (lottieValueCallback == null) {
    ...
    public void drawLayer(Canvas canvas, Matrix matrix, int i3) {
        ...
        if (alpha != 0) {
            int intValue = (int) ((((((float) alpha) / 255.0f) * ((float) (this.transform.getOpacity() == null ? 100 : this.transform.getOpacity().getValue().intValue()))) / 100.0f) * (((float) i3) / 255.0f) * 255.0f);
            this.paint.setAlpha(intValue);
            BaseKeyframeAnimation<Integer, Integer> baseKeyframeAnimation = this.colorAnimation;
            if (baseKeyframeAnimation != null) {
                this.paint.setColor(baseKeyframeAnimation.getValue().intValue());
            }
