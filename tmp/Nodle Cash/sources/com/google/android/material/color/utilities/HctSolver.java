...
public class HctSolver {
    ...
    public static double[] bisectToLimit(double d2, double d3) {
        int i3;
        int i4;
        double[][] bisectToSegment = bisectToSegment(d2, d3);
        ...
        for (int i5 = 0; i5 < 3; i5++) {
            ...
            if (d4 != d5) {
                if (d4 < d5) {
                    i4 = criticalPlaneBelow(trueDelinearized(d4));
                    i3 = criticalPlaneAbove(trueDelinearized(dArr2[i5]));
                } else {
                    i4 = criticalPlaneAbove(trueDelinearized(d4));
                    i3 = criticalPlaneBelow(trueDelinearized(dArr2[i5]));
                }
                int i6 = i4;
                int i7 = i3;
                for (int i8 = 0; i8 < 8 && Math.abs(i7 - i6) > 1; i8++) {
                    ...
                    int i9 = floor;
                    if (areInCyclicOrder(hueOf, d3, hueOf2)) {
                        dArr2 = coordinate;
                        i7 = i9;
                    } else {
                        dArr = coordinate;
    ...
    public static double[][] bisectToSegment(double d2, double d3) {
        ...
        for (int i3 = 0; i3 < 12; i3++) {
            double[] nthVertex = nthVertex(d2, i3);
            if (nthVertex[0] >= 0.0d) {
                ...
                if (!z2) {
                    ...
                    d4 = hueOf;
                    d5 = d4;
                } else if (z3 || areInCyclicOrder(d4, hueOf, d5)) {
                    if (areInCyclicOrder(d4, d3, hueOf)) {
                        z3 = false;
                        dArr2 = nthVertex;
                        d5 = hueOf;
                    } else {
    ...
    public static double chromaticAdaptation(double d2) {
        ...
        return ((((double) MathUtils.signum(d2)) * 400.0d) * pow) / (pow + 27.13d);
    }
    ...
    public static int findResultByJ(double d2, double d3, double d4) {
        ...
        while (true) {
            ...
            double d7 = ncb;
            double pow2 = Math.pow(((d3 == 0.0d || sqrt == 0.0d) ? 0.0d : d3 / Math.sqrt(d6)) * pow, 1.1111111111111112d);
            double pow3 = (Math.pow(d6, (d5 / viewingConditions.getC()) / viewingConditions.getZ()) * viewingConditions.getAw()) / viewingConditions.getNbb();
            double d8 = (((0.305d + pow3) * 23.0d) * pow2) / (((pow2 * 108.0d) * sin) + (((pow2 * 11.0d) * cos) + (d7 * 23.0d)));
            double d9 = d8 * cos;
            double d10 = d8 * sin;
            double d11 = pow3 * 460.0d;
            matrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation(((288.0d * d10) + ((451.0d * d9) + d11)) / 1403.0d), inverseChromaticAdaptation(((d11 - (891.0d * d9)) - (261.0d * d10)) / 1403.0d), inverseChromaticAdaptation(((d11 - (d9 * 220.0d)) - (d10 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            double d12 = matrixMultiply[0];
            if (d12 < 0.0d) {
                break;
            }
            double d13 = matrixMultiply[1];
            if (d13 < 0.0d) {
                break;
            }
            double d14 = matrixMultiply[2];
            if (d14 >= 0.0d) {
    ...
    public static double inverseChromaticAdaptation(double d2) {
        ...
        return Math.pow(Math.max(0.0d, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d) * ((double) MathUtils.signum(d2));
    }
    ...
    public static double[] lerpPoint(double[] dArr, double d2, double[] dArr2) {
        double d3 = dArr[0];
        double a2 = a.a(dArr2[0], d3, d2, d3);
        double d4 = dArr[1];
        double a3 = a.a(dArr2[1], d4, d2, d4);
        double d5 = dArr[2];
        return new double[]{a2, a3, a.a(dArr2[2], d5, d2, d5)};
    }
    ...
    public static double[] nthVertex(double d2, int i3) {
        int i4 = i3;
        double[] dArr = Y_FROM_LINRGB;
        double d3 = dArr[0];
        double d4 = dArr[1];
    ...
    public static double[] setCoordinate(double[] dArr, double d2, double[] dArr2, int i3) {
        return lerpPoint(dArr, intercept(dArr[i3], d2, dArr2[i3]), dArr2);
    }
    ...
    public static int solveToInt(double d2, double d3, double d4) {
        ...
        double sanitizeDegreesDouble = (MathUtils.sanitizeDegreesDouble(d2) / 180.0d) * 3.141592653589793d;
        double yFromLstar = ColorUtils.yFromLstar(d4);
        int findResultByJ = findResultByJ(sanitizeDegreesDouble, d3, yFromLstar);
