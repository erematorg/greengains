...
final class AesGcmProtoSerialization {
    ...
    public static /* synthetic */ class AnonymousClass1 {
        ...
        static {
            ...
            L_0x0033:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: com.google.crypto.tink.aead.AesGcmProtoSerialization.AnonymousClass1.<clinit>():void");
        }
    }
    ...
    static {
        ...
        PARAMETERS_SERIALIZER = ParametersSerializer.create(new a(4), AesGcmParameters.class, cls);
        ...
        KEY_SERIALIZER = KeySerializer.create(new a(6), AesGcmKey.class, cls2);
    ...
    public static AesGcmKey parseKey(ProtoKeySerialization protoKeySerialization, @Nullable SecretKeyAccess secretKeyAccess) throws GeneralSecurityException {
        if (protoKeySerialization.getTypeUrl().equals(TYPE_URL)) {
            try {
                AesGcmKey parseFrom = AesGcmKey.parseFrom(protoKeySerialization.getValue(), ExtensionRegistryLite.getEmptyRegistry());
    ...
    public static ProtoKeySerialization serializeKey(AesGcmKey aesGcmKey, @Nullable SecretKeyAccess secretKeyAccess) throws GeneralSecurityException {
        ...
        return ProtoKeySerialization.create(TYPE_URL, ((AesGcmKey) AesGcmKey.newBuilder().setKeyValue(ByteString.copyFrom(aesGcmKey.getKeyBytes().toByteArray(SecretKeyAccess.requireAccess(secretKeyAccess)))).build()).toByteString(), KeyData.KeyMaterialType.SYMMETRIC, toProtoOutputPrefixType(aesGcmKey.getParameters().getVariant()), aesGcmKey.getIdRequirementOrNull());
    }
    ...
    private static void validateParameters(AesGcmParameters aesGcmParameters) throws GeneralSecurityException {
        if (aesGcmParameters.getTagSizeBytes() != 16) {
            throw new GeneralSecurityException(String.format("Invalid tag size in bytes %d. Currently Tink only supports serialization of AES GCM keys with tag size equal to 16 bytes.", new Object[]{Integer.valueOf(aesGcmParameters.getTagSizeBytes())}));
        } else if (aesGcmParameters.getIvSizeBytes() != 12) {
            throw new GeneralSecurityException(String.format("Invalid IV size in bytes %d. Currently Tink only supports serialization of AES GCM keys with IV size equal to 12 bytes.", new Object[]{Integer.valueOf(aesGcmParameters.getIvSizeBytes())}));
        }
    }
