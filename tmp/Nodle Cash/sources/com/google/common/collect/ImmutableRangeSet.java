...
public final class ImmutableRangeSet<C extends Comparable> extends AbstractRangeSet<C> implements Serializable {
    ...
    public final class AsSet extends ImmutableSortedSet<C> {
        ...
        public int indexOf(@CheckForNull Object obj) {
            ...
            while (it.hasNext()) {
                ...
                if (range.contains(comparable)) {
                    return Ints.saturatedCast(j2 + ((long) ContiguousSet.create(range, this.domain).indexOf(comparable)));
                }
                j2 += (long) ContiguousSet.create(range, this.domain).size();
            }
        ...
        public int size() {
            ...
            if (num == null) {
                ...
                long j2 = 0;
                while (it.hasNext()) {
                    j2 += (long) ContiguousSet.create((Range) it.next(), this.domain).size();
        ...
        public ImmutableSortedSet<C> subSetImpl(C c3, boolean z2, C c4, boolean z3) {
            if (z2 || z3 || Range.compareOrThrow(c3, c4) != 0) {
                return subSet(Range.range(c3, BoundType.forBoolean(z2), c4, BoundType.forBoolean(z3)));
            }
    ...
    public static class Builder<C extends Comparable<?>> {
        ...
        public ImmutableRangeSet<C> build() {
            ...
            return build.isEmpty() ? ImmutableRangeSet.of() : (build.size() != 1 || !((Range) Iterables.getOnlyElement(build)).equals(Range.all())) ? new ImmutableRangeSet<>(build) : ImmutableRangeSet.all();
        }
    ...
    public final class ComplementRanges extends ImmutableList<Range<C>> {
        ...
        public Range<C> get(int i3) {
            ...
            if (this.positiveBoundedBelow) {
                cut = i3 == 0 ? Cut.belowAll() : ((Range) ImmutableRangeSet.this.ranges.get(i3 - 1)).upperBound;
            } else {
                cut = ((Range) ImmutableRangeSet.this.ranges.get(i3)).upperBound;
            }
            if (!this.positiveBoundedAbove || i3 != this.size - 1) {
                cut2 = ((Range) ImmutableRangeSet.this.ranges.get(i3 + (this.positiveBoundedBelow ^ true ? 1 : 0))).lowerBound;
            } else {
                cut2 = Cut.aboveAll();
            }
    ...
    private ImmutableList<Range<C>> intersectRanges(final Range<C> range) {
        ...
        return binarySearch2 == 0 ? ImmutableList.of() : new ImmutableList<Range<C>>() {
            ...
            public Range<C> get(int i3) {
                ...
                if (i3 == 0 || i3 == binarySearch2 - 1) {
                    return ((Range) ImmutableRangeSet.this.ranges.get(i3 + binarySearch)).intersection(range);
                }
                return (Range) ImmutableRangeSet.this.ranges.get(i3 + binarySearch);
            }
        };
    }
    ...
    public boolean intersects(Range<C> range) {
        ...
        if (binarySearch > 0) {
            ...
            return this.ranges.get(i3).isConnected(range) && !this.ranges.get(i3).intersection(range).isEmpty();
        }
    }
