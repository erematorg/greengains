...
public interface Detector<DetectionResultT> extends Closeable, LifecycleObserver {
    ...
    @NonNull
    @KeepForSdk
    Task<DetectionResultT> process(@NonNull Bitmap bitmap, int i3);

    @NonNull
    @KeepForSdk
    Task<DetectionResultT> process(@NonNull Image image, int i3);

    @NonNull
    @KeepForSdk
    Task<DetectionResultT> process(@NonNull Image image, int i3, @NonNull Matrix matrix);

    @NonNull
    @KeepForSdk
    Task<DetectionResultT> process(@NonNull ByteBuffer byteBuffer, int i3, int i4, int i5, int i6);
}
