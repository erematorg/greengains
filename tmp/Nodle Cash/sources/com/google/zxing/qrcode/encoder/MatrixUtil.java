...
final class MatrixUtil {
    ...
    public static void buildMatrix(BitArray bitArray, ErrorCorrectionLevel errorCorrectionLevel, Version version, int i3, ByteMatrix byteMatrix) throws WriterException {
    ...
    public static int calculateBCHCode(int i3, int i4) {
        if (i4 != 0) {
            int findMSBSet = findMSBSet(i4);
            int i5 = i3 << (findMSBSet - 1);
    ...
    public static void embedDataBits(BitArray bitArray, int i3, ByteMatrix byteMatrix) throws WriterException {
        boolean z2;
            ...
            while (height >= 0 && height < byteMatrix.getHeight()) {
                for (int i6 = 0; i6 < 2; i6++) {
                    ...
                    if (isEmpty(byteMatrix.get(i7, height))) {
                        ...
                        if (i3 != -1 && MaskUtil.getDataMaskBit(i3, i7, height)) {
                            z2 = !z2;
                        }
                        byteMatrix.set(i7, height, z2);
                    }
                }
                height += i5;
            }
        ...
        if (i4 != bitArray.getSize()) {
            throw new WriterException("Not all bits consumed: " + i4 + JsonPointer.SEPARATOR + bitArray.getSize());
        }
    }

    private static void embedHorizontalSeparationPattern(int i3, int i4, ByteMatrix byteMatrix) throws WriterException {
        int i5 = 0;
        while (i5 < 8) {
            int i6 = i3 + i5;
            if (isEmpty(byteMatrix.get(i6, i4))) {
                byteMatrix.set(i6, i4, 0);
                i5++;
            } else {
                throw new WriterException();
            }
    ...
    private static void embedPositionAdjustmentPattern(int i3, int i4, ByteMatrix byteMatrix) {
        for (int i5 = 0; i5 < 5; i5++) {
            int[] iArr = POSITION_ADJUSTMENT_PATTERN[i5];
            for (int i6 = 0; i6 < 5; i6++) {
                byteMatrix.set(i3 + i6, i4 + i5, iArr[i6]);
            }
        }
    }

    private static void embedPositionDetectionPattern(int i3, int i4, ByteMatrix byteMatrix) {
        for (int i5 = 0; i5 < 7; i5++) {
            int[] iArr = POSITION_DETECTION_PATTERN[i5];
    ...
    private static void embedTimingPatterns(ByteMatrix byteMatrix) {
        int i3 = 8;
        while (i3 < byteMatrix.getWidth() - 8) {
            int i4 = i3 + 1;
            int i5 = i4 % 2;
            if (isEmpty(byteMatrix.get(i3, 6))) {
                byteMatrix.set(i3, 6, i5);
            }
            if (isEmpty(byteMatrix.get(6, i3))) {
                byteMatrix.set(6, i3, i5);
            }
            i3 = i4;
        }
    }

    public static void embedTypeInfo(ErrorCorrectionLevel errorCorrectionLevel, int i3, ByteMatrix byteMatrix) throws WriterException {
        int i4;
        BitArray bitArray = new BitArray();
        makeTypeInfoBits(errorCorrectionLevel, i3, bitArray);
        for (int i5 = 0; i5 < bitArray.getSize(); i5++) {
            boolean z2 = bitArray.get((bitArray.getSize() - 1) - i5);
            int[] iArr = TYPE_INFO_COORDINATES[i5];
            byteMatrix.set(iArr[0], iArr[1], z2);
            int i6 = 8;
            if (i5 < 8) {
                i4 = 8;
                i6 = (byteMatrix.getWidth() - i5) - 1;
            } else {
                i4 = (i5 - 8) + (byteMatrix.getHeight() - 7);
            }
            byteMatrix.set(i6, i4, z2);
        }
    }

    private static void embedVerticalSeparationPattern(int i3, int i4, ByteMatrix byteMatrix) throws WriterException {
        int i5 = 0;
        while (i5 < 7) {
            int i6 = i4 + i5;
            if (isEmpty(byteMatrix.get(i3, i6))) {
                byteMatrix.set(i3, i6, 0);
                i5++;
            } else {
                throw new WriterException();
            }
    ...
    public static int findMSBSet(int i3) {
        return 32 - Integer.numberOfLeadingZeros(i3);
    }

    private static boolean isEmpty(int i3) {
        return i3 == -1;
    }

    public static void makeTypeInfoBits(ErrorCorrectionLevel errorCorrectionLevel, int i3, BitArray bitArray) throws WriterException {
        if (QRCode.isValidMaskPattern(i3)) {
    ...
    private static void maybeEmbedPositionAdjustmentPatterns(Version version, ByteMatrix byteMatrix) {
        if (version.getVersionNumber() >= 2) {
            ...
            for (int i3 : iArr) {
                if (i3 >= 0) {
                    for (int i4 : iArr) {
                        if (i4 >= 0 && isEmpty(byteMatrix.get(i4, i3))) {
        ...
        if (version.getVersionNumber() >= 7) {
            ...
            for (int i4 = 0; i4 < 6; i4++) {
                for (int i5 = 0; i5 < 3; i5++) {
                    ...
                    i3--;
                    byteMatrix.set(i4, (byteMatrix.getHeight() - 11) + i5, z2);
                    byteMatrix.set((byteMatrix.getHeight() - 11) + i5, i4, z2);
                }
            }
        }
    }
